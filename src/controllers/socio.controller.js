...
export const postSocio = async (req, res) => {
  try {
    const Query =
      "INSERT INTO OP_Socio (SOC_Name,SOC_Telefono,SOC_CreatedDateTime,SOC_ModifiedDateTime,SOC_ModifiedBy,SOC_Active) VALUES (?,?,?,?,?,?)";
    const Values = [
      req.body.SOC_Name,
...
export const putSocio = async (req, res) => {
  try {
    ...
    const Query =
      "UPDATE OP_Socio SET SOC_Name = ? ,SOC_Telefono = ?,SOC_ModifiedDateTime = ?,SOC_ModifiedBy = ?, SOC_Active = ? WHERE SOC_Id = ?";
    const Values = [
      req.body.SOC_Name,
      req.body.SOC_Telefono,
      new Date(),
...
export const DelSocio = async (req, res) => {
  try {
    const Query =
      "UPDATE OP_Socio SET SOC_ModifiedDateTime = ?,SOC_ModifiedBy = ?, SOC_Active = ? WHERE SOC_Id = ?";
    const Values = [
      new Date(),
      req.body.SOC_ModifiedBy,
      req.body.SOC_Active,

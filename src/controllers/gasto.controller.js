...
export const getGasto = async (req, res) => {
  try {
    const Querys =
      "SELECT GAT_Id,GAT_Date,GAT_SOC_Id,SOC_Name,GAT_OSB_Id,OSB_Nombre,GAT_OSBT_Id, OSBT_Nombre,GAT_Reason,\
...
export const postGasto = async (req, res) => {
  try {
    ...
    const Querys =
      "INSERT INTO STD_Gastos (GAT_Date,GAT_SOC_Id,GAT_OSB_Id,GAT_OSBT_Id,GAT_Reason,GAT_Amount,\
            GAT_CreatedDateTime,GAT_ModifiedDateTime, GAT_ModifiedBy,GAT_Active) VALUES(?,?,?,?,?,?,?,?,?,?)";
    const Values = [
      Date1,
      req.body.GAT_SOC_Id,
      req.body.GAT_OSB_Id,
      req.body.GAT_OSBT_Id,
      req.body.GAT_Reason,
      req.body.GAT_Amount,
      Date2,
      Date2,
      req.body.GAT_ModifiedBy,
      req.body.GAT_Active,
    ];
...
export const putGasto = async (req, res) => {
  try {
    ...
    const Querys =
      "UPDATE STD_Gastos SET GAT_Date = ?, GAT_SOC_Id = ?, GAT_OSB_Id = ?, GAT_OSBT_Id = ?, GAT_Reason = ?,\
...
export const delGasto = async (req, res) => {
  try {
    ...
    const Querys =
      "UPDATE STD_Gastos SET GAT_ModifiedDateTime = ?, GAT_ModifiedBy = ?, GAT_Active = ? WHERE GAT_Id = ?";
    const Values = [
      Date2,
      req.body.GAT_ModifiedBy,
      req.body.GAT_Active,
